...
test("vol.002 sixth revision is complete and keeps every earlier edition separate", async () => {
  ...
  assert.match(detail, /characters-revised6-pencil\.png/);
  assert.match(detail, /人物画は画像生成/);
  for (const [chapter, asset] of [["04", "studio-revised6-pencil.png"]]) {
...
test("all four originals remain intact and each edition has independent URLs and progress", async () => {
  ...
  for (const book of originals) {
    ...
    assert.equal(new Set(editions.map(b => b.readBase)).size, book.id === "mukae-no-nai-asa" ? 7 : book.id === "ame-wo-tojikomeru" ? 4 : book.id === "hako-no-soto-de-machiawase" ? 3 : 2);
    const revised = current.find(b => b.id === book.id);
    assert.equal(revised.edition, book.id === "mukae-no-nai-asa" ? "revised-20260920-6" : ["ame-wo-tojikomeru", "hako-no-soto-de-machiawase"].includes(book.id) ? "revised-20260920-3" : "revised-20260918");
...
test("archived revisions preserve prose, metadata and saved reading locations", async () => {
  ...
  assert.equal(new Set(versions.map(b => progressKey(id, b.edition))).size, 7);
  ...
  assert.ok(detail.includes("第六改稿版（最新版）"));
  ...
  const newChapter = await fs.readFile(path.join(root, "dist/books", id, "read/revised-20260920-6/15.html"), "utf8");
  assert.match(newChapter, /第十四章　片方の補助輪/);
  const currentOpening = await fs.readFile(path.join(root, "dist/books", id, "read/revised-20260920-6/01.html"), "utf8");
  assert.ok(currentOpening.includes('data-edition="revised-20260920-6"'));
});
